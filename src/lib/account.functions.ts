...
export const deleteAccount = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    if (anonErr) {
      ...
      throw new Error("Kunde inte radera kontot — försök igen eller mejla info@hpkampen.se.");
    }
    ...
    if (authErr) {
      ...
      if (banErr) {
        ...
        throw new Error(
          "Dina uppgifter är raderade men kontot kunde inte stängas helt — mejla info@hpkampen.se så fixar vi det.",
        );
      }
    }
